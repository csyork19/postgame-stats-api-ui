...
function PlayerSeasonAverageStats() {
    ...
    useEffect(() => {
        ...
        if (selectedPlayer) {
            fetch('http://127.0.0.1:5000/api/nba/player/perSeasonStats', {
                method: 'POST',
                headers: {
    ...
    return (
        ...
        <TableContainer component={Paper} style={{ width: '75%', backgroundColor: '#FAFDF3' }}>
            <h1 style={{ textAlign: 'center', marginTop: 0 }}>Player Season Average Stats - {data.playerName} | {data.season}</h1>
